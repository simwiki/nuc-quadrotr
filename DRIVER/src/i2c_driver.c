...
** @date 2016-11-20
** @decription 
**              this file provides driving functions to use i2c
**              When defined USE_I2C_STANDARD_LIBRARY,we use st's standard 
**              library to drive i2c.
**              when defined USE_IO_SIMULATE_I2C,we use st's standard library
**              to drive i2c.
**************************************************************************************
++ @attention  
++              change your i2cx in i2c_driver.h and change your i2c's gpio
++              in i2c_driver.h.
**************************************************************************************/
 ...
 ************************************************************************************************************/
 ...
 int8_t mI2C_SendByte(I2C_Typedef* I2Cx,uint8_t Slave_addr,uint8_t Reg_addr,uint8_t len,uint8_t *Send_Buff)
 {
	 ...
	 I2C_GenerateSTART(I2Cx,ENABLE);    //I2C generate start signal
	 ...
	 I2C_Send7biteAddress(I2Cx,Slave_addr,I2C_Direction_Transmitter);     //Send Address
	 uint8_t Wait_Time = 0xff;
	 ...
	 I2C_SendData(I2Cx,Reg_addr);           //Connection is successful and Send Regsiter Address
	 uint8_t Wait_Time = 0xff;
	...
	{	                    //if Connection between master and slaver is successful,then send Data Buff
	 ...
	 I2C_GenerateSTOP(I2Cx,ENABLE);                    //Send Data Finish Stop I2C Bus
...
************************************************************************************************************/
 ...
 {
	 uint16_t Wait_Time = 0x1ff;                           //Check I2C whether is busy
	 ...
	 I2C_GenerateSTART(I2Cx,ENABLE);                        //Generate a start signal
	 ...
	 I2C_Send7biteAddress(I2Cx,Slave_Addr,I2C_Direction_Transmitter); //Send Slaver address when send regsiter address
	 uint8_t Wait_Time = 0xff;
	 while( !I2C_CheckEvent ( I2Cx,I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED ) && Wait_Time-- );
		 if ( -1 == Wait_Time )
		 {
	 ...
	 while(len)
	{
		 if ( 1 == len )
			 {
			 I2C_AcknowledgeConfig(I2Cx,DISABLE);                   //generate a no ack when read the last Byte
			 I2C_GenerateSTOP(I2Cx,ENABLE);
	     }
...
{
	mI2C_GPIO_Configeration(m_I2C);
	return mI2C_WriteByte(Slave_Addr, Reg_Addr, len, Send_Buff);
}
...
{
	mI2C_GPIO_Configeration(m_I2C);
  return mI2C_ReadByte(Slave_Addr, Reg_Addr, len, Recv_Buff); 	
}
